...
void SkinningEditor::DrawImGui() {
    ...
    if (!ImGui::Begin("スキニングエディタ (Skinning Editor)", &isOpen_)) {
        ImGui::End();
        return;
    }

    if (ImGui::Button("全モデル削除 (Delete All Models)")) {
        ClearTargets();
    }
    ImGui::Separator();
    ...
    if (!targets_.empty()) {
        ImGui::TextUnformatted("登録モデル (Registered Models)");
    ...
    if (!targets_.empty()) {
        ...
        ImGui::Combo("対象モデル (Target)", &currentTargetIndex_, targetLabels.data(), static_cast<int>(targetLabels.size()));
        SyncTargetFromIndex();
    } else {
        ImGui::Text("対象モデル (Target): %s", targetLabel_.c_str());
    }
    ...
    ImGui::SeparatorText("対象情報 (Target Debug)");
    if (activeTarget) {
        ImGui::Text("現在の対象 (Active Target): %s", activeTarget->label.c_str());
        ImGui::Text("対象種類 (Target Type): %s", activeTarget->targetType.c_str());
        ImGui::TextWrapped("読込パス (Resolved Path): %s", activeTarget->resolvedPath.empty() ? "(なし)" : activeTarget->resolvedPath.c_str());
        ImGui::Text("スケルトン骨数 (Skeleton Bone Count): %d", activeTarget->boneCount);
        ImGui::Text("スキンメッシュ (Skinned Mesh): %s", activeTarget->skinnedMeshLoaded ? "読込済み (loaded)" : "未読込 (not loaded)");
        ...
        if (activeTarget->skinnedMeshLoaded) {
            ...
            ImGui::Text("最終ワールドスケール (Final Object World Scale): %.4f", activeTarget->previewInfo.previewScale);

            Vector3 previewRotationDegrees = RadiansToDegrees(activeTarget->previewInfo.previewRotation);
            if (ImGui::DragFloat3("スキン表示回転 deg (Skinned Preview Rotation)", &previewRotationDegrees.x, 1.0f, -360.0f, 360.0f, "%.1f")) {
                activeTarget->previewInfo.previewRotation = DegreesToRadians(previewRotationDegrees);
            }
            ImGui::SameLine();
            if (ImGui::Button("回転を初期化 (Reset Rotation)")) {
            ...
            ImGui::Text(
                "最終ワールド回転 (Final Object World Rotation): %.1f, %.1f, %.1f deg",
            ...
            if (sourceBounds.isValid) {
                ImGui::Text(
                    "ローカル最小 (Local Min): %.3f, %.3f, %.3f",
                    sourceBounds.min.x,
                    sourceBounds.min.y,
                    sourceBounds.min.z);
                ImGui::Text(
                    "ローカル最大 (Local Max): %.3f, %.3f, %.3f",
                    sourceBounds.max.x,
                    sourceBounds.max.y,
                    sourceBounds.max.z);
                ImGui::Text(
                    "ローカルサイズ (Local Size): %.3f, %.3f, %.3f",
                    sourceBounds.size.x,
                    sourceBounds.size.y,
                    sourceBounds.size.z);
                ImGui::Text(
                    "ローカル中心 (Local Center): %.3f, %.3f, %.3f",
            ...
            if (skinnedBounds.isValid) {
                ImGui::Text(
                    "初期スキンサイズ (Initial Skinned Size): %.3f, %.3f, %.3f",
            ...
            ImGui::Text(
                "初期ルート位置 (Initial Skeleton Root World T): %.3f, %.3f, %.3f",
            ...
            ImGui::Text(
                "初期ルート拡縮 (Initial Skeleton Root World S): %.4f, %.4f, %.4f",
            ...
            if (targetSkeleton_ &&
                ...
                const Vector3 displayedRootPosition = TransformPosition(
                    ...
                    GetActivePreviewWorldMatrix());
                ImGui::Text(
                    "現在ルート位置 (Current Skeleton Root World T): %.3f, %.3f, %.3f",
                    currentRoot.worldTranslate.x,
                    currentRoot.worldTranslate.y,
                    currentRoot.worldTranslate.z);
                ImGui::Text(
                    "現在ルート拡縮 (Current Skeleton Root World S): %.4f, %.4f, %.4f",
                    currentRootScale.x,
                    currentRootScale.y,
                    currentRootScale.z);
                ImGui::Text(
                    "表示ルート拡縮 (Displayed Root Scale): %.4f, %.4f, %.4f",
                    displayedRootScale.x,
                    displayedRootScale.y,
                    displayedRootScale.z);
                ImGui::Text(
                    "表示ルート位置 (Displayed Root T): %.3f, %.3f, %.3f",
        ...
        if (!activeTarget->loadStatus.empty()) {
            ImGui::TextWrapped("読込結果 (Load Result): %s", activeTarget->loadStatus.c_str());
        }
    }
    ...
    ImGui::InputText("JSON保存先 (JSON Path)", jsonPathBuffer_.data(), jsonPathBuffer_.size());

    if (ImGui::Button("JSON保存 (Save JSON)")) {
        SaveCurrentClipToJson(false);
    }
    ImGui::SameLine();
    if (ImGui::Button("新規JSON保存 (Save As New JSON)")) {
    ...
    ImGui::DragFloat("再生速度 (Playback Speed)", &playbackSpeed_, 0.01f, 0.0f, 4.0f, "%.2f");
    ImGui::Text("再生状態 (Playback State): %s", !hasClip_ ? "クリップなし (No Clip)" : (isPlaying_ ? (isPaused_ ? "一時停止 (Paused)" : "再生中 (Playing)") : "停止中 (Stopped)"));
    ImGui::Checkbox("ギズモ有効 (Enable Gizmo)", &isTranslateGizmoEnabled_);
    int gizmoOperationIndex = static_cast<int>(gizmoOperation_);
    if (ImGui::Combo("ギズモ操作 (Gizmo Operation)", &gizmoOperationIndex, kGizmoOperationLabels, IM_ARRAYSIZE(kGizmoOperationLabels))) {
    ...
    if (ImGui::Combo("ギズモ空間 (Gizmo Space)", &gizmoSpaceIndex, kGizmoSpaceLabels, IM_ARRAYSIZE(kGizmoSpaceLabels))) {
        gizmoSpace_ = static_cast<GizmoSpace>(gizmoSpaceIndex);
    }
    if (!isTranslateOperation) {
        ImGui::EndDisabled();
        ImGui::TextDisabled("回転 / 拡縮ギズモはローカル固定です (Rotate / Scale uses Local).");
    }

    ImGui::SeparatorText("スケルトン表示補助 (Skeleton Debug Overlay)");
    ImGui::Checkbox("ジョイント点を表示 (Show Joint Points)", &showSkeletonDebugJoints_);
    ImGui::Checkbox("親子線を表示 (Show Parent-Child Lines)", &showSkeletonDebugLines_);
    ImGui::Checkbox("ジョイント名を表示 (Show Joint Labels)", &showJointLabels_);
    ...
    if (selectedJointIndex_ >= 0 && selectedJointIndex_ < static_cast<int>(targetSkeleton_->joints.size())) {
        ...
        ImGui::Text("選択Index (Selected Index): %d", selectedJointIndex_);
        ImGui::Text("選択ボーン名 (Selected Bone Name): %s", joint.name.c_str());
        ImGui::Text("ギズモ操作中 (Gizmo Active): %s", isGizmoActive_ ? "はい (Yes)" : "いいえ (No)");
        const char* gizmoOperationLabel = kGizmoOperationLabels[static_cast<int>(gizmoOperation_)];
        ImGui::Text("ギズモ操作 (Gizmo Operation): %s", gizmoOperationLabel);
        const bool isTranslateOperationInInspector = (gizmoOperation_ == GizmoOperation::Translate);
        ImGui::Text(
            "ギズモ空間 (Gizmo Space): %s",
            isTranslateOperationInInspector
            ? (gizmoSpace_ == GizmoSpace::Local ? "ローカル (Local)" : "ワールド (World)")
            : "ローカル固定 (Rotate / Scale fixed)");
        ImGui::Text("親Index (Parent Index): %d", joint.parentIndex);
        ImGui::Text("子ボーン数 (Children): %d", static_cast<int>(joint.children.size()));
        const Vector3 displayWorldPosition = TransformPosition(joint.worldTranslate, GetActivePreviewWorldMatrix());
        ImGui::Text("ワールド位置 (World Pos): %.2f, %.2f, %.2f", joint.worldTranslate.x, joint.worldTranslate.y, joint.worldTranslate.z);
        ...
        if (ImGui::DragFloat3("ローカル回転 (Local Rotate)", &localRotateDegrees.x, 1.0f)) {
            joint.localRotate = {
        ...
        if (ImGui::DragFloat3("ローカル拡縮 (Local Scale)", &joint.localScale.x, 0.01f, 0.01f, 10.0f, "%.2f")) {
            isJointEdited = true;
        }
...
void SkinningEditor::DrawTimelineWindow() {
    ...
    if (!ImGui::Begin("スキニングタイムライン (Skinning Timeline)")) {
        ImGui::End();
        return;
    }
...
void SkinningEditor::DrawTransport() {
    ...
    if (ImGui::SliderFloat("現在時刻 (Current Time)", &currentTime_, 0.0f, clipDuration, "%.3f")) {
        if (hasClip_) {
            ApplyCurrentClipAtCurrentTime();
        }
    }

    if (ImGui::Button("現在時刻にキー追加 (Add Key)")) {
        AddKeyAtCurrentTime();
    }
    ImGui::SameLine();
    ...
    if (ImGui::Button("選択キー削除 (Delete Selected Key)")) {
        DeleteSelectedKey();
    }
    if (!canDeleteKey) {
        ImGui::EndDisabled();
...
void SkinningEditor::DrawTimelineCanvas() {
    ...
    drawList->AddText({ labelMinX, canvasPosition.y + 6.0f }, IM_COL32(220, 220, 230, 255), "Bone / ボーン");
    drawList->AddText({ timelineMinX, canvasPosition.y + 6.0f }, IM_COL32(220, 220, 230, 255), "Timeline / 時間軸");
...
void SkinningEditor::DrawSelectedJointKeys() {
    ...
    ImGui::Text("ジョイント (Joint): %s", joint.name.c_str());

    JointTrack* track = GetSelectedJointTrack();
    if (!track || track->keys.empty()) {
        ImGui::TextDisabled("このジョイントにキーはありません (No keys).");
        return;
    }
    ...
    for (int keyIndex = 0; keyIndex < static_cast<int>(track->keys.size()); ++keyIndex) {
        const Keyframe& key = track->keys[static_cast<size_t>(keyIndex)];
        char label[96] = {};
        sprintf_s(label, "キー (Key) %d  |  t = %.3f", keyIndex, key.time);
...
void SkinningEditor::DrawSelectedKeyInspector() {
    ...
    if (!track || selectedKeyIndex_ < 0 || selectedKeyIndex_ >= static_cast<int>(track->keys.size())) {
        ImGui::TextDisabled("選択ボーンのキーから選んでください (Select a key).");
        return;
    }

    Keyframe& key = track->keys[static_cast<size_t>(selectedKeyIndex_)];
    float keyTime = key.time;
    const bool keyTimeChanged = ImGui::DragFloat("選択キー時刻 (Selected Key Time)", &keyTime, 0.01f, 0.0f, 600.0f, "%.3f");
    ...
    ImGui::Text("移動 (Translate): %.2f, %.2f, %.2f", key.translate.x, key.translate.y, key.translate.z);
    ImGui::Text("回転 (Rotate): %.2f, %.2f, %.2f", key.rotate.x, key.rotate.y, key.rotate.z);
    ImGui::Text("拡縮 (Scale): %.2f, %.2f, %.2f", key.scale.x, key.scale.y, key.scale.z);
#endif
}
...
void SkinningEditor::DrawDebugOverlay(const Camera* camera) const {
    ...
    for (int jointIndex = 0; jointIndex < static_cast<int>(targetSkeleton_->joints.size()); ++jointIndex) {
        ...
        if (showJointLabels_ && isSelected) {
            const char* activeOperationLabel = gizmoOperation_ == GizmoOperation::Translate
                ? "[移動 Translate] 選択ボーン"
                : (gizmoOperation_ == GizmoOperation::Rotate ? "[回転 Rotate] 選択ボーン" : "[拡縮 Scale] 選択ボーン");
            const char* selectedLabel = isGizmoActiveJoint ? activeOperationLabel : "選択ボーン (Selected Bone)";
    ...
    if (showSelectedJointPanel_ && hasSelectedJointScreen && selectedJointIndex_ >= 0 && selectedJointIndex_ < static_cast<int>(targetSkeleton_->joints.size())) {
        ...
        std::string title = "選択 (Selected): " + selectedJoint.name;
        const bool isTranslateOperation = (gizmoOperation_ == GizmoOperation::Translate);
        const char* stateLabel = gizmoOperation_ == GizmoOperation::Translate
            ? (isGizmoActive_ ? "移動ギズモ操作中 (Translate)" : "移動ギズモ待機 (Translate)")
            : (gizmoOperation_ == GizmoOperation::Rotate
                ? (isGizmoActive_ ? "回転ギズモ操作中 (Rotate)" : "回転ギズモ待機 (Rotate)")
                : (isGizmoActive_ ? "拡縮ギズモ操作中 (Scale)" : "拡縮ギズモ待機 (Scale)"));
        const char* spaceLabel = isTranslateOperation
            ? (gizmoSpace_ == GizmoSpace::Local ? "空間: ローカル (Local)" : "空間: ワールド (World)")
            : "空間: ローカル (Local)";
